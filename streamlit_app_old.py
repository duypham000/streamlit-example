...
def label(text_to_summarize):
    prompt = label_promp(text_to_summarize.split())
    txt_word_count = len(text_to_summarize.split())
...
def action(text_to_action):
    ...
    arr_label = []
    tpc_anchor = 0
    while tpc_anchor < len(split_res):
        arr_split_label = []
        while len(arr_split_label) != len(split_res[tpc_anchor:5]):
            print(len(split_res))
            print(len(arr_label))
            mat_label = "\n- ".join(split_res[tpc_anchor:5]).strip()
            raw_label = label(mat_label).strip()
            arr_split_label = re.findall("Info|Positive|Negative", raw_label)
        tpc_anchor += 4
        arr_label += arr_split_label
